...
public class StreamUploadOutputStream extends OutputStream implements StreamObserver<DownMessage> {
    ...
    public void close() throws IOException {
		...
		try {
			if ( m_state == State.UPLOADING ) {
				...
				s_logger.info("finished uploading: size={}, elapsed={}, velo={}/s",
								UnitUtils.toByteSizeString(m_offsetHead),
								m_watch.getElapsedMillisString(), UnitUtils.toByteSizeString(velo));
			}
			else {
    ...
    private void writeChunk(byte bytes[], int offset, int size) throws IOException {
		...
		try {
			...
			throw new IOException("uploading thread has been interrupted");
		}
		finally {
			m_guard.unlock();
		}
