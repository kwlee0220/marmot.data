 ...
 */
public class AvroBinaryRecordReader extends AbstractRecordStream {
	...
	public static RecordStream deserialize(RecordSchema schema, Schema avroSchema, byte[] bytes) {
		ByteArrayInputStream bais = new ByteArrayInputStream(bytes);
		return new AvroBinaryRecordReader(schema, avroSchema, bais);
	}
	
	public static RecordStream deserialize(RecordSchema schema, Schema avroSchema, InputStream is) throws IOException {
		return new AvroBinaryRecordReader(schema, avroSchema, is);
	}
	
	public static RecordStream deserialize(RecordSchema schema, Schema avroSchema, File file) throws IOException {
		InputStream is = new BufferedInputStream(new FileInputStream(file));
		return new AvroBinaryRecordReader(schema, avroSchema, is);
	}
	
	private AvroBinaryRecordReader(RecordSchema schema, Schema avroSchema, InputStream is) {
